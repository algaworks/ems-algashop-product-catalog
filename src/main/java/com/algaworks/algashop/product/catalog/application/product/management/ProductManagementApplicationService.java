...
import com.algaworks.algashop.product.catalog.domain.model.stock.StockMovement;
import com.algaworks.algashop.product.catalog.domain.model.stock.StockMovementRepository;
import com.algaworks.algashop.product.catalog.domain.model.stock.StockService;
...
public class ProductManagementApplicationService {
    ...
    public void restock(UUID productId, int quantity) {
        Product product = findProduct(productId);
        StockMovement movement = stockService.restock(product.getId(), quantity);
    ...
    public void withdraw(UUID productId, int quantity) {
        Product product = findProduct(productId);
        StockMovement movement = stockService.withdraw(product.getId(), quantity, null);
        stockMovementRepository.save(movement);
    }
